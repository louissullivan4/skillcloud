...
const MsgApply = () => {
    ...
    const AcceptApply= async () => {
        const resp = await fetch('http://127.0.0.1:5000/applyresponse/'+from.project_author+"/"+from.user_notified+"/"+from.role_id+"/accepted")
        const data = await resp.status;
        navigate("/completerequest" , { state: { from: data } });
    };

    const DeclineApply = async () => {
        const resp = await fetch('http://127.0.0.1:5000/applyresponse/'+from.project_author+"/"+from.user_notified+"/"+from.role_id+"/declined")
        const data = await resp.status;
        navigate("/completerequest" , { state: { from: data } });
    };
