...
const MsgResponse = () => {
    ...
    const addUser = async () => {
        const resp = await fetch('http://scbackend-env.eba-imjep3am.eu-north-1.elasticbeanstalk.com/rolechange/'+from.user_notified+"/"+from.role_id+"/add")
        const data = await resp.json();
    ...
    const removeUser = async () => {
        const resp = await fetch('http://scbackend-env.eba-imjep3am.eu-north-1.elasticbeanstalk.com/rolechange/'+from.user_notified+"/"+from.role_id+"/remove")
        const data = await resp.json();
        setCompletion(data.result);
        navigate("/completerequest" , { state: { from: completion } });
    };
